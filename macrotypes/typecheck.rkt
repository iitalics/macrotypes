  ...
  (define (infer es #:ctx [ctx null] #:tvctx [tvctx null]
     ...
     (syntax-parse ctx
       ...
       #:do [; use internal-definition-context to rebind ctx
             ...
             (define (flip* es)
               (stx-map flip es))
             (define (expand+intdef e)
               (local-expand e 'expression '() intdef))

             ; bind tyvars tv => tv-
             (for ([tv (in-syntax #'[tv ...])]
                   [tv- (in-syntax #'[tv- ...])]
                   [tv* (in-syntax #`[(mk-tyvar (attachs #'tv- '(tvsep ...) #'(tvk ...)
                                                         #:ev #,kev))
                                      ...])])
               (bind tv- #f)
               (bind tv #`(make-rename-transformer #,tv*)))
             ...
             (for ([x (in-syntax #'[X ... x ...])]
                   ...
                   [x* (in-syntax #`[(mk-tyvar (attach #'X- ':: (#,kev #'#%type)))
                                     ...
                                     ...])])
               (bind x- #f)
               (bind x #`(make-variable-like-transformer #,x*)))

             ]
       ; local expand expressions using internal definition context
       #:with es- (stx-map expand+intdef es)

       ; (list tvs- xs- es- τs)
       (list (flip* #'(tv- ...))
             (flip* #'(X- ... x- ...))
             #'es-
             (stx-map (λ (e) (detach e tag)) #'es-))]
